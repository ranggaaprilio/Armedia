<!-- Content Header (Page header) -->
<div class="dashboard-wrapper">
    <div class="dashboard-ecommerce">
        <div class="container-fluid dashboard-content ">
            <!-- ============================================================== -->
            <!-- pageheader  -->
            <!-- ============================================================== -->
            <!-- biodata Pasien -->
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-header " style=" background-color: #00897b !important;">
                        <h4 style="font-size: 25px;" class="text-white my-auto">Biodata Pasien</h4>
                    </div>
                    <div class="card-body">
                        <?php foreach ($daftar as $d) { ?>
                            <table border="0">
                                <tr>
                                    <td width="50%">No Rawat</td>
                                    <td><?php echo $d->no_rawat ?></td>
                                </tr>
                                <tr>
                                    <td width="50%">No Rekamedis</td>
                                    <td>
                                        <?php
                                        $no_rm=$d->no_rekamedis;
                                        echo $d->no_rekamedis;
                                        ?>
                                        <?php $no_rm = $d->no_rekamedis; ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="50%">Nama Pasien</td>
                                    <td>
                                        <?php
                                        $pasien = $this->Base_model->get_data_where('pasien', 'no_rekamedis', $d->no_rekamedis)->row();
                                        echo $pasien->nama_pasien;
                                        $nama=$pasien->nama_pasien;
                                        $kategori=$d->kategori;
                                        ?>
                                    </td>
                                </tr>
                            </table>
                        
                        <hr>
                        <div class="row">
                            <div class="col-1 col-sm-3 col-lg-2">
                                <a class="btn btn-primary btn-sm" href="#" role="button" data-toggle="modal" data-target="#exampleModalCenter">
                                    <p>Input Tindakan</p>
                                </a>
                            </div>
                            <div class="col-1 col-sm-3 col-lg-2">
                                <a class="btn btn-primary btn-sm" href="#" role="button" data-toggle="modal" data-target="#exampleModal">
                                    <p>Input Obat</p>
                                </a>
                            </div>
                             <div class="col-1 col-sm-3 col-lg-2">
                                <a class="btn btn-success btn-sm" href="#" role="button" data-toggle="modal" data-target="#exampleModalLong">
                                    <p>Detail Identitas</p>
                                </a>
                            </div>
                            <div class="col-1 col-sm-3 col-lg-2">
                                <a class="btn btn-danger btn-sm" href="<?= base_url('dokter/selesai/'). $d->no_rawat ?>" role="button">
                                    <p>Selesai</p>
                                </a>
                            </div>
                            <?php } ?>
                        </div>
                    </div>
                </div>
            </div>
            <!-- end biodata pasien -->

            <!-- Modal Identitas -->

                <div class="modal fade" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                  <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Identitas Lengkap</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
                      <div class="modal-body">
                        <?php 
                        $pasien=$this->Base_model->get_data_where('pasien','no_rekamedis',$no_rm)->result(); 
                        foreach ($pasien as $p) {?>
                       <div class="row">
                           <div class="mx-auto">
                               <img src="<?php echo base_url('assets/img/user/').$p->foto; ?>" alt="None Pict" width="200px" height="100">
                           </div>
                       </div>
                       <hr>
                       <label for="Nama">No KTP</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->no_ktp ?>" readonly>
                       <label for="Nama">Nama Pasien</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->nama_pasien ?>" readonly>
                       <label for="Tempat lahir">Tempat Lahir</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->tempat_lahir ?>" readonly>
                       <label for="Tanggal Lahir">Tanggal Lahir</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->tanggal_lahir ?>" readonly>
                       <label for="No_telp">No telpon</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->no_telp ?>" readonly>
                        <label for="Alamat">Alamat</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->alamat ?>" readonly>
                       <label for="Alamat">Email</label>
                       <input class="form-control" type="text" placeholder="<?php echo $p->email ?>" readonly>

                        <?php }
                        ?>
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                       
                      </div>
                    </div>
                  </div>
                </div>

            <!-- Modal Tindakan-->
            <div class="modal fade  bd-example-modal-lg" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h3 class="modal-title" id="exampleModalCenterTitle">Input Riwayat Rekamedis <?= $nama?></h3><br>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <?php
                            foreach ($daftar as $e) {
                                ?>
                                <?php echo form_open('dokter/input_rekam_hamil'); ?>
                                <input type="hidden" value="<?php echo $e->no_rekamedis ?>" name="no_rm">
                                <input type="hidden" value="<?php echo $e->no_rawat ?>" name="no_rawat">
                                <?php $no_rm = $e->no_rekamedis ?>
                                <!-- Nama Lengkap -->
                                <div class="form-group">
                                    <label for="inputUserName">Dokter Penanggung jawab</label>
                                    <input id="inputUserName" type="text" name="id" placeholder="Masukan Nama Lengkap" autocomplete="off" class="form-control" value="<?php echo $e->id_dokter ?>" readonly>
                                    <?= form_error('id', '<small class="text-danger pl-4">', '</small>') ?>
                                </div>
                                <!-- Tanggal -->
                                <div class="form-group">
                                    <label>Tanggal Periksa</label>
                                    <input type="text" name="tanggal" placeholder="Masukan tanggal" class="form-control" autocomplete="off" value="<?php echo $e->tanggal_daftar ?>" readonly>
                                    <?= form_error('tanggal', '<small class="text-danger pl-4">', '</small>') ?>
                                </div>
                                <!-- Trimester -->
                                <div class="form-group">
                                    <label>Trimester</label>
                                    <select class="form-control" id="input-select" name='trimester'>
                                        <option value="">-PILIH-</option>
                                        <option value="0">Tidak Mengandung</option>
                                        <option value="1">Trimester 1</option>
                                        <option value="2">Trimester 2</option>
                                        <option value="3">Trimester 3</option>
                                    </select>
                                    <?= form_error('trimester', '<small class="text-danger pl-4">', '</small>') ?>
                                </div>

                                <!-- Hasil -->
                                <div class="form-group">
                                    <label for="input-select">Hasil Periksa</label>
                                    <textarea name="hasil" id="hasil" cols="10" rows="5" class="form-control" placeholder="Masukan Hasil Periksa"></textarea>
                                    <?= form_error('hasil', '<small class="text-danger pl-4">', '</small>') ?>
                                </div>
                                <!-- Tindakan -->
                                <div class="form-group">
                                    <label for="input-select">Tindakan Yang dilakukan</label>
                                    <textarea name="tindakan" id="tindakan" cols="10" rows="5" class="form-control" placeholder="Masukan Tindakan"></textarea>
                                    <?= form_error('tindakan', '<small class="text-danger pl-4">', '</small>') ?>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class=" fas fa-arrow-right"></i></button>
                                <button type="submit" class="btn btn-space btn-warning  " style="background-color:#00897b !important"><i class="fas fa-save text-white"></i></button>
                                <?php form_close(); ?>
                            <?php } ?>
                        </div>
                    </div>
                </div>
            </div>

            <!-- end Modal -->

            <!-- Modal obat -->
            
            <div class="modal fade bd-example-modal-lg" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Input Obat</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="modal-body">
                    <!-- obat -->
                             <div class="table-responsive">
                            <table class="table table-striped table-bordered" id="table-datatable" style="width:100%;">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>ID Obat</th>
                                        <th>Nama Obat</th>
                                        <th>Jenis Obat</th>
                                        <th>Dosis Obat</th>
                                        <th>Satuan</th>
                                        <th>Pilihan</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $no = 1;
                                    foreach ($obat as $o) {
                                        ?>
                                        <tr>
                                            <td><?php echo $no++; ?></td>
                                            <td><?php echo $o->id_obat ?></td>
                                            <td><?php echo $o->nama_obat; ?></td>
                                            <td><?php echo $o->jenis_obat; ?></td>
                                            <td><?php echo $o->dosis_aturan_obat; ?></td>
                                            <td><?php echo $o->satuan; ?></td>
                                            <td class="nowrap">
                                                <?php foreach ($daftar as $k) {?>
                                                   <a href="<?php echo base_url().'dokter/masukan_obat/' . $o->id_obat.'/'.$k->no_rekamedis.'/'.$k->kategori.'/'.$k->no_rawat ?>" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a>
                                                <?php } ?>
                                               
                                               
                                            </td>
                                        </tr>
                                    <?php
                                } ?>
                                </tbody>
                            </table>
                        </div>        
                    </div>
                </div>
              </div>
            </div>

            <!-- Riwayat Rekamedis -->
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-header " style=" background-color: #00897b !important;">
                        <h4 style="font-size: 25px;" class="text-white my-auto">Riwayat Tindakan</h4>
                    </div>
                    <div class="card-body">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">No</th>
                                    <th scope="col">No Rawat</th>
                                    <th scope="col">ID Dokter</th>
                                    <th scope="col">Trimester</th>
                                    <th scope="col">Tindakan</th>
                                    <th scope="col">Hasil Periksa</th>
                                    <th scope="col">Tanggal</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $riwayat = $this->Base_model->get_data_where('tindakan_hamil', 'no_rekamedis', $no_rm)->result();
                                $no = 1;
                                foreach ($riwayat as $d) {
                                    ?>
                                    <tr>
                                        <td><?php echo $no++; ?></td>
                                        <td><?php echo $d->no_rawat ?></td>
                                        <td><?php echo $d->id_dokter; ?></td>
                                        <td><?php echo $d->trimester; ?></td>
                                        <td><?php echo $d->nama_tindakan; ?></td>
                                        <td><?php echo $d->hasil_periksa; ?></td>
                                        <td><?php echo $d->tanggal; ?></td>
                                    </tr>
                                <?php
                            } ?>
                            </tbody>
                        </table>
                    </div>
                    <!-- end card body -->
                </div>
                <!-- end card -->
            </div>
               <!-- end riwayat rekamedis -->

               <!-- Riwayat OBat -->
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-header " style=" background-color: #00897b !important;">
                        <h4 style="font-size: 25px;" class="text-white my-auto">Riwayat Obat</h4>
                    </div>
                    <div class="card-body">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">No</th>
                                    <th scope="col">No Rawat</th>
                                    <th scope="col">Nama Obat</th>
                                    <th scope="col">Jenis</th>
                                    <th scope="col">Dosis</th>
                                    <th scope="col">Satuan</th>
                                </tr>
                            </thead>
                            <tbody>
                        <?php 
                        $where = array('no_rekamedis' => $no_rm ,'kategori'=>$kategori );

                        $temp=$this->Base_model->edit_data($where,'temp_obat')->result();
                        $no=1;
                        foreach ($temp as $t ) {
                            $id_obat=$t->id_obat;
                            $obat=$this->Base_model->get_data_where('obat','id_obat',$id_obat)->result();

                            foreach ($obat as $o) {?>
                                <tr>
                                        <td><?php echo $no++ ?></td>
                                        <td><?php echo $t->no_rawat ?></td>
                                        <td><?php echo $o->nama_obat ?></td>
                                        <td><?php echo $o->jenis_obat ?></td>
                                        <td><?php echo $o->dosis_aturan_obat ?></td>
                                        <td><?php echo $o->satuan?></td>   

                                </tr>
                                
                                      
                          <?php  }
                        }

                          ?>
                            </tbody>
                        </table>
                    </div>
                    <!-- end card body -->
                </div>
                <!-- end card -->
            </div>
               <!-- end riwayat rekamedis -->
        </div>

     


        <!-- ============================================================== -->
        <!-- end pageheader  -->
        <!-- ============================================================== -->

    </div>
</div>